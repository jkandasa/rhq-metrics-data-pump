...
public class CommandDispatchingInputController extends AnInputController{
	...
	public boolean handle(final Context ctx) {
		final String cmdLine = (String) ctx.getValue(Context.KEY_COMMAND_LINE_INPUT);
		_logger.debug("Processing command line [{}] ...", cmdLine);

		if ((cmdLine == null) || cmdLine.trim().isEmpty() || this.commands.isEmpty()) {
			_logger.warn("No command line given or no commands registered - will abort processing");
			return false;
		}
		...
		if (cmd == null) {
			_logger.debug("Unknown command [{}] - abort processing", tokens[0]);
			ctx.getIoConsole().writeOutput(String.format("%nCommand [%s] is unknown. Type help for a list of installed commands.%n%n", tokens[0]));
			return false;
		}
		if (tokens.length > 1) {
		...
		cmd.execute(ctx);
		_logger.debug("Finished processing command line [{}] - using command [{}]", cmdLine, cmd);
		return true;
	}
	...
	public void plug(final Context plug) {
		super.plug(plug);
		final List<Command> allCommands = plug.getCommandsByNamespace(ConsoleCommands.NAMESPACE_RHQ_METRICS_CLI);
		if (allCommands.size() > 0) {
			this.commands.putAll(plug.mapCommands(allCommands));
			_logger.info("Registered known commands: [{}]", this.commands);
			...
			plug.getIoConsole().writeOutput(String.format("%nNo commands were found for input controller [%s].%nn", this.getClass().getName()));
		}
	}
